...
dark      = readtable('counter_trace_stokes_dark_counts.csv');
...
alpha1_eff = ((0.378*2/11.795)+(0.355*9.795/11.795)); % 1310 nm
alpha1 = alpha1_eff/4.343;
alpha2_eff = ((0.185*2/11.795)+(0.248*9.795/11.795)); % 1550 nm
...
for p = 1:length(potencias)
    ...
    d0 = readtable(['stokes_sin_distancia_' sufijo '.csv']);
    d2km = readtable(['stokes_2km_' sufijo '.csv']);
    d10km = readtable(['stokes_10km_' sufijo '.csv']);
    d12km = readtable(['stokes_12km_' sufijo '.csv']);
    ...
    title(['Scattering Raman Stokes (' sufijo ')'])
...
for p = 1:length(potencias)
    ...
    % Cómputo de resultados de estimación y error 
    fprintf('%2d dBm: beta = %.3e ± %.3e km^-1   MSE = %.3e W^2\n',...
        potencias_dBm(p),...
        beta_ind(p),...
        sigma_ind(p),...
        MSE_ind(p));
...
title('Modelo Scattering Raman Stokes frente a medidas experimentales','FontSize',16)
